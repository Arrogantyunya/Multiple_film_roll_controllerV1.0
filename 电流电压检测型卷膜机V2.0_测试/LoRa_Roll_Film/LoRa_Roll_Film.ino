...
void setup()
{
	...
	Motor_Operation.Motor_GPIO_Config();//配置电机相关引脚
	Motor_Operation.Direction_Selection(Stop);//选择电机转向或停止
	...
	Roll_Timer_Init();//使用定时器2初始化卷膜行程计时参数
	...
	while (SN.Self_check(gSN_Code) == false)
	{
		...
		Serial.println("验证SN代码失败，尝试检索SN代码…");
	...
	Set_Motor_Status(NOT_INITIALIZED);//设置当前设备工作状态

	Motor_Operation.Adjust_deviation();//非正常断电后，恢复先前目标开度
	iwdg_feed();
...
void loop()
{
	...
	Motor_Operation.Detect_Manual_Rolling();//检测手动卷膜
	Motor_Operation.Trace_Opening();//开度追踪机制

	Motor_Operation.Adjust_Opening();//开度误差校正

	Check_Store_Param_And_LoRa();//检查存储参数以及LORA参数

	Key_Clear_Current_Value();//按键清除当前值
}
...
void Request_Access_Network(void)
{
	if (SN.Verify_SN_Access_Network_Flag() == false)
	{
		...
		if (SN.Save_SN_Code(gSN_Code) && SN.Save_BKP_SN_Code(gSN_Code))
			Serial.println("Write SN success...写入SN成功");
		...
		if (Roll_Operation.Save_Group_Number(Default_WorkGroup))
			Serial.println("Save gourp number success...保存组ID成功");
...
void Key_Clear_Current_Value(void)
{
	if (digitalRead(SW_FUN2) == LOW)
	{
		...
		{
			Some_Peripheral.Key_Buzz(600);
			Serial.println("Prepare clear current threshold...准备清除当前阈值");
